...
public:
  ...
  bool CompareExchangeWeak(
      Flags<Enum> &expected, Flags<Enum> desired, std::memory_order order = std::memory_order_seq_cst);
  bool CompareExchangeStrong(
      Flags<Enum> &expected, Flags<Enum> desired, std::memory_order order = std::memory_order_seq_cst);
  bool CompareExchangeWeak(
      Flags<Enum> &expected, Flags<Enum> desired, std::memory_order success, std::memory_order failure);
  bool CompareExchangeStrong(
      Flags<Enum> &expected, Flags<Enum> desired, std::memory_order success, std::memory_order failure);
...
template <typename Enum>
constexpr Flags<Enum>::Flags(Enum value) noexcept : value_(static_cast<ValueType>(value)) {}

template <typename Enum>
constexpr Flags<Enum>::Flags(std::initializer_list<Enum> values) noexcept : Flags() {
  for (Enum value : values) { *this |= value; }
}
...
template <typename Enum>
AtomicFlags<Enum> &AtomicFlags<Enum>::Store(Flags<Enum> rhs, std::memory_order order) {
...
template <typename Enum>
Flags<Enum> AtomicFlags<Enum>::FetchOr(Flags<Enum> rhs, std::memory_order memory_order) {
  return static_cast<Enum>(value_.fetch_or(rhs.value_, memory_order));
}

template <typename Enum>
Flags<Enum> AtomicFlags<Enum>::FetchAnd(Flags<Enum> rhs, std::memory_order memory_order) {
  return static_cast<Enum>(value_.fetch_and(rhs.value_, memory_order));
}

template <typename Enum>
Flags<Enum> AtomicFlags<Enum>::FetchClear(Flags<Enum> flags, std::memory_order memory_order) {
  return static_cast<Enum>(value_.fetch_and(~flags.value_, memory_order));
}

template <typename Enum>
bool AtomicFlags<Enum>::CompareExchangeWeak(Flags<Enum> &expected, Flags<Enum> desired, std::memory_order order) {
  auto expected_int = expected.GetValue();
  const bool result = value_.compare_exchange_weak(expected_int, desired.GetValue(), order);
  expected          = Enum {expected_int};
  return result;
}

template <typename Enum>
bool AtomicFlags<Enum>::CompareExchangeStrong(Flags<Enum> &expected, Flags<Enum> desired, std::memory_order order) {
  auto expected_int = expected.GetValue();
  const bool result = value_.compare_exchange_strong(expected_int, desired.GetValue(), order);
  expected          = Enum {expected_int};
  return result;
}

template <typename Enum>
bool AtomicFlags<Enum>::CompareExchangeWeak(
    Flags<Enum> &expected, Flags<Enum> desired, std::memory_order success, std::memory_order failure) {
  auto expected_int = expected.GetValue();
  const bool result = value_.compare_exchange_weak(expected_int, desired.GetValue(), success, failure);
  expected          = Enum {expected_int};
  return result;
}

template <typename Enum>
bool AtomicFlags<Enum>::CompareExchangeStrong(
    Flags<Enum> &expected, Flags<Enum> desired, std::memory_order success, std::memory_order failure) {
  auto expected_int = expected.GetValue();
  const bool result = value_.compare_exchange_strong(expected_int, desired.GetValue(), success, failure);
  expected          = Enum {expected_int};
  return result;
}
