...
using OstreamWriteResult =
    decltype(std::declval<std::ostream &>() << std::declval<const std::remove_reference_t<T> &>());
...
template <typename T>
inline constexpr bool kIsUniqueMap = kIsMap<T> && kIsDetected<impl::SubscriptOperatorResult,
