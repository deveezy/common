...
constexpr std::chrono::duration<Rep, Period> NormalizeTimeOfDay(std::chrono::duration<ORep, OPeriod> d) {
  auto res = std::chrono::duration_cast<std::chrono::duration<Rep, Period>>(d) % kTwentyFourHours<Rep, Period>;
...
struct HasMinutes<std::ratio<Num, Den>> : std::integral_constant<bool, (Num <= 60LL)> {};
...
template <intmax_t Num, intmax_t Den>
struct HasSeconds<std::ratio<Num, Den>> : std::integral_constant<bool, (Num == 1)> {};
...
template <intmax_t Num, intmax_t Den>
struct HasSubseconds<std::ratio<Num, Den>> : std::integral_constant<bool, (Den > 1)> {};

template <typename Rep, typename Period>
struct HasSubseconds<std::chrono::duration<Rep, Period>> : HasSubseconds<Period> {};
  ...
  constexpr DurationType operator()(std::string_view str) {
      ...
      switch (c) {
        case ':':
          if (position_ >= kSeconds)
            throw std::runtime_error {std::format("Extra colon in TimeOfDay string `{}`", str)};
          AssignCurrentPosition(str);
          position_ = static_cast<TimePart>(position_ + 1);
          break;
        case '.':
          if (position_ != kSeconds)
            throw std::runtime_error {std::format("Unexpected decimal point in TimeOfDay string `{}`", str)};
          AssignCurrentPosition(str);
          position_ = static_cast<TimePart>(position_ + 1);
          break;
        default:
          if (!std::isdigit(c))
            throw std::runtime_error {std::format("Unexpected character {} in TimeOfDay string `{}`", c, str)};
          if (position_ == kOverflow) {
            ...
            throw std::runtime_error {std::format("Too much digits in TimeOfDay string `{}`", str)};
          }
          ++digit_count_;
    ...
    if (position_ == kHour)
      throw std::runtime_error {
          std::format("Expected to have at least minutes after hours in "
                      "TimeOfDay string `{}`",
              str)};
    AssignCurrentPosition(str);
    ...
    if (sum > kTwentyFourHours<Rep, Period>) {
      throw std::runtime_error(std::format("TimeOfDay value {} is out of range [00:00, 24:00)", str));
    }
...
private:
  ...
  void AssignCurrentPosition(std::string_view str) {
    switch (position_) {
      case kHour: {
        if (digit_count_ < 1)
          throw std::runtime_error {std::format("Not enough digits for hours in TimeOfDay string `{}`", str)};
        if (current_number_ > 24)
          throw std::runtime_error {std::format("Invalid value for hours in TimeOfDay string `{}`", str)};
        hours_ = std::chrono::hours {current_number_};
        break;
      }
      case kMinutes: {
        if (digit_count_ != 2)
          throw std::runtime_error {std::format("Not enough digits for minutes in TimeOfDay string `{}`", str)};
        if (current_number_ > 59)
          throw std::runtime_error {std::format("Invalid value for minutes in TimeOfDay string `{}`", str)};
        minutes_ = std::chrono::minutes {current_number_};
        break;
      }
      case kSeconds: {
        if (digit_count_ != 2)
          throw std::runtime_error {std::format("Not enough digits for seconds in TimeOfDay string `{}`", str)};
        if (current_number_ > 59)
          throw std::runtime_error {std::format("Invalid value for seconds in TimeOfDay string `{}`", str)};
        seconds_ = std::chrono::seconds {current_number_};
        break;
      }
      case kSubseconds: {
        if (digit_count_ < 1)
          throw std::runtime_error {std::format("Not enough digits for subseconds in TimeOfDay string `{}`", str)};
        if constexpr (kHasSubseconds<Period>) {
          // TODO check digit count and adjust if needed
          if (digit_count_ < kDecimalPositions<Period>) {
            current_number_ *= MissingDigits(kDecimalPositions<Period> - digit_count_);
          }
...
template <>
inline constexpr std::string_view kSubsecondsPreformat<std::milli> = ".{:0>#3d}";
template <>
inline constexpr std::string_view kSubsecondsPreformat<std::micro> = ".{:0>#6d}";
template <>
...
template <typename Rep, typename Period>
constexpr TimeOfDay<std::chrono::duration<Rep, Period>>::TimeOfDay(DurationType d) noexcept
    : since_midnight_ {detail::NormalizeTimeOfDay<Rep, Period>(d)} {}

template <typename Rep, typename Period>
template <typename ORep, typename OPeriod>
constexpr TimeOfDay<std::chrono::duration<Rep, Period>>::TimeOfDay(std::chrono::duration<ORep, OPeriod> d) noexcept
    : since_midnight_ {detail::NormalizeTimeOfDay<Rep, Period>(d)} {}
...
constexpr TimeOfDay<std::chrono::duration<Rep, Period>> TimeOfDay<std::chrono::duration<Rep, Period>>::FromHHMMInt(
  ...
  auto mm = hh_mm % 100;
  if (mm >= 60)
    throw std::runtime_error {std::format("Invalid value for minutes {} in int representation {}", mm, hh_mm)};
...
class formatter<utils::datetime::TimeOfDay<Duration>> {
  /// Format string used for format key `%H`, two-digit 24-hour left-padded by 0
  static constexpr auto kLongHourFormat = utils::datetime::detail::kLongHourFormat;
  /// Format string used for minutes, key `%M`, no variations
  static constexpr auto kMinutesFormat = utils::datetime::detail::kMinutesFormat;
  /// Format string used for seconds, key `%S`, no variations
  static constexpr auto kSecondsFormat = utils::datetime::detail::kSecondsFormat;
  /// Format string for subseconds, keys not yet assigned
  /// for use in representation
  static constexpr auto kSubsecondsFormat = utils::datetime::detail::kSubsecondsFormat;

  static constexpr auto kSubsecondsPreformat = utils::datetime::detail::kSubsecondsPreformat<typename Duration::period>;

  static constexpr std::string_view kLiteralPercent = "%";

  static constexpr auto kDefaultFormat = utils::datetime::detail::kDefaultFormat<typename Duration::period>;

  constexpr std::string_view GetFormatForKey(char key) {
    // TODO Check if time part already seen
    switch (key) {
      case 'H': return kLongHourFormat;
      case 'M': return kMinutesFormat;
      case 'S': return kSecondsFormat;
      default: throw format_error {std::format("Unsupported format key {}", key)};
    }
  }
...
public:
  constexpr auto parse(format_parse_context &ctx) {
    ...
    for (; it != end && *it != '}'; ++it) {
      ...
      if (*it == '%') {
        if (state == kPercent) {
          ...
          if (state == kChar && size > 0) { PushBackFmt({begin, size}); }
          state = kPercent;
        }
      } else {
        if (state == kPercent) {
    ...
    if (!custom_format) {
      for (const auto fmt : kDefaultFormat) { PushBackFmt(fmt); }
    }
    if (state == kChar) {
      if (size > 0) { PushBackFmt({begin, size}); }
  ...
  constexpr auto format(const utils::datetime::TimeOfDay<Duration> &value, FormatContext &ctx) const {
    ...
    constexpr std::size_t buffer_size =
        std::max(utils::datetime::detail::kDecimalPositions<typename Duration::period>, std::size_t {1}) + 2;
    char subseconds[buffer_size];
    subseconds[0] = 0;
    if (ss > 0 || !truncate_trailing_subseconds_) {
      std::format_to(subseconds, kSubsecondsPreformat, ss);
    ...
    for (std::size_t i = 0; i < representation_size_; ++i) {
      res = format_to(ctx.out(), fmt::runtime(representation_[i]), hours, mins, secs, subseconds);
    }
    return res;
  }
