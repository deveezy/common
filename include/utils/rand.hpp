...
T RandRange(T from_inclusive, T to_exclusive) {
  INVARIANT(from_inclusive < to_exclusive, "attempt to get a random value in an incorrect range");
  if constexpr (std::is_floating_point_v<T>) {
    ...
    return utils::WithDefaultRandom(
        std::uniform_int_distribution<std::common_type_t<T, unsigned short>> {from_inclusive, to_exclusive - 1});
  }
}
...
void Shuffle(Container &container) {
  utils::WithDefaultRandom(
      [&container](RandomBase &rng) { std::shuffle(std::begin(container), std::end(container), rng); });
}
