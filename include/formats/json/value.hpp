...
#include <formats/json/schema.hpp>
...
public:
  ...
  /// pointer.
  bool DebugIsReferencingSameMemory(const Value &other) const { return value_ptr_ == other.value_ptr_; }
...
public:
  /// @cond
  Value(EmplaceEnabler, const impl::VersionedValuePtr &root, const impl::Value *root_ptr_for_path,
      const impl::Value *value_ptr, int depth);

  Value(EmplaceEnabler, const impl::VersionedValuePtr &root, impl::Value *root_ptr_for_path,
...
private:
  ...
  public:
    ...
    LazyDetachedPath(impl::Value *parent_value_ptr, int parent_depth, std::string_view key);
