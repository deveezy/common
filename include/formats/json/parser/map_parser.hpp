...
public:
  ...
  void StartObject() override {
    switch (state_) {
      case State::kStart: this->state_ = State::kInside; break;

      case State::kInside: this->Throw("{");
    }
  }
  ...
  std::string Expected() const override {
    switch (state_) {
      case State::kInside: return "string";

      case State::kStart: return "object";
    }

    INVARIANT(false, "Unexpected parser state");
  }

private:
  void OnSend(Value &&value) override { this->result_.emplace(std::move(key_), std::move(value)); }
