...
ObjectType ParseObject(const Value &value, ExtractFunc &&extract_func) {
  ...
  for (auto it = value.begin(); it != value.end(); ++it) { result.emplace(it.GetName(), extract_func(*it)); }
...
T Parse(const Value &value, To<T>)
  requires(common::kIsFormatValue<Value> && meta::kIsRange<T> && !meta::kIsMap<T> &&
...
template <typename T, typename Value>
T Parse(const Value &value, To<T>)
  requires(common::kIsFormatValue<Value> && meta::kIsMap<T>)
{
