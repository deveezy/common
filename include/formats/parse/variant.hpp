...
[[noreturn]] void ThrowVariantAmbiguousParse(const std::string &path, std::type_index type_b) {
  throw ParseException("Value of '" + path + "' is ambiguous, it is parseable into multiple variants of '" +
...
auto Parse(const Value &value, formats::parse::To<std::variant<Types...>>) {
  ...
  if (!result) { ThrowVariantParseException<typename Value::ParseException, std::variant<Types...>>(value.GetPath()); }
