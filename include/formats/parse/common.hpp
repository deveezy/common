...
void CheckInBounds(const Value &value, T x, T min, T max) {
  if (x < min || x > max) {
    throw typename Value::ParseException(
        fmt::format("Value of '{}' is out of bounds ({} <= {} <= {})", value.GetPath(), min, x, max));
  }
}
...
Dst NarrowToInt(Src x, const Value &value) {
  static_assert(std::numeric_limits<Src>::min() <= std::numeric_limits<Dst>::min() &&
...
std::chrono::seconds ToSeconds(const std::string &data, const Value &value) {
  ...
  if (converted != ms) {
    throw
        typename Value::ParseException(fmt::format("Value of '{}' = {}ms cannot be represented as "
...
std::enable_if_t<common::kIsFormatValue<Value>, std::chrono::duration<double, Period>> Parse(
    const Value &n, To<std::chrono::duration<double, Period>>) {
