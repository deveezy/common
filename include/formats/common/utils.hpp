...
ValueBuilder GetAtPath(ValueBuilder &parent, std::vector<std::string> &&path, std::size_t path_size) {
  INVARIANT(path_size != 0, "attempt to get a ValueBuilder element on an empty path");
  if (path_size == 1) { return parent[std::move(path[0])]; }
  ...
  requires(common::kIsFormatValue<Value>)
{
  ...
  for (const auto &current_key : path) { current_value = current_value[current_key]; }
