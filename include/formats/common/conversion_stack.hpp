...
public:
  ...
  void CastCurrentPrimitive() {
    ASSERT(!stack_.front().is_parsed && !stack_.back().is_parsed);
  ...
  void EnterItems() {
    ...
    if (current_docs.from.IsObject()) {
      ...
      ASSERT_MSG(false, "Type mismatch");
    }
    stack_.pop_back();
  }
...
ValueTo PerformMinimalFormatConversion(ValueFrom &&value) {
  if (value.IsMissing()) {
    throw typename std::decay_t<ValueFrom>::Exception(
        fmt::format("Failed to convert value at '{}' from {} to {}: missing value", value.GetPath(),
            compiler::GetTypeName<std::decay_t<ValueFrom>>(), compiler::GetTypeName<ValueTo>()));
  }
  ...
  while (!conversion_stack.IsParsed()) {
    ...
    if (from.IsBool()) {
      ...
      throw typename std::decay_t<ValueFrom>::Exception(
          fmt::format("Failed to convert value at '{}' from {} to {}: unknown node type", from.GetPath(),
              compiler::GetTypeName<std::decay_t<ValueFrom>>(), compiler::GetTypeName<ValueTo>()));
    }
  }
