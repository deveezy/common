...
TEST(UnderlyingValue, EmptyStruct) {
  EXPECT_TRUE((std::is_same_v<int, decltype(utils::UnderlyingValue(IntEnum(1)))>));
  EXPECT_TRUE((std::is_same_v<unsigned, decltype(utils::UnderlyingValue(UnsignedEnum(1)))>));
  EXPECT_TRUE((std::is_same_v<short, decltype(utils::UnderlyingValue(UnscopedEnum::kValue))>));
}
