...
TEST(TrivialBiMap, StringICase) {
  ...
  EXPECT_EQ(kICaseCheck.TryFindICase("\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a\x1b"
                                     "\x1c\x1d\x1e\x1f"),
      7);
  EXPECT_EQ(kICaseCheck.TryFindICase("\x80\x81\x82\x83\x84\x85\x86\x87\x88\x89\x8a\x8b"
                                     "\x8c\x8d\x8e\x8f"),
      8);
  EXPECT_EQ(kICaseCheck.TryFindICase("\x90\x91\x92\x93\x94\x95\x96\x97\x98\x99\x9a\x9b"
                                     "\x9c\x9d\x9e\x9f"),
      9);
  EXPECT_EQ(kICaseCheck.TryFindICase("\xa0\xa1\xa2\xa3\xa4\xa5\xa6\xa7\xa8\xa9\xaa\xab"
                                     "\xac\xad\xae\xaf"),
      10);
  EXPECT_EQ(kICaseCheck.TryFindICase("\xb0\xb1\xb2\xb3\xb4\xb5\xb6\xb7\xb8\xb9\xba\xbb"
                                     "\xbc\xbd\xbe\xbf"),
      11);
  EXPECT_EQ(kICaseCheck.TryFindICase("\xc0\xc1\xc2\xc3\xc4\xc5\xc6\xc7\xc8\xc9\xca\xcb"
                                     "\xcc\xcd\xce\xcf"),
      12);
  EXPECT_EQ(kICaseCheck.TryFindICase("\xd0\xd1\xd2\xd3\xd4\xd5\xd6\xd7\xd8\xd9\xda\xdb"
                                     "\xdc\xdd\xde\xdf"),
      13);
  EXPECT_EQ(kICaseCheck.TryFindICase("\xe0\xe1\xe2\xe3\xe4\xe5\xe6\xe7\xe8\xe9\xea\xeb"
                                     "\xec\xed\xee\xef"),
      14);
  EXPECT_EQ(kICaseCheck.TryFindICase("\xf0\xf1\xf2\xf3\xf4\xf5\xf6\xf7\xf8\xf9\xfa\xfb"
                                     "\xfc\xfd\xfe\xff"),
      15);
...
TEST(TrivialBiMap, ConstexprIteration) {
  constexpr auto sum = []() {
    ...
    int sum = 0;
    for (const auto &[literal, i] : kMap) { sum += i; }
...
TEST(TrivialBiMap, Empty) {
  constexpr auto sum = []() {
    ...
    int sum = 0;
    for (const auto &[literal, i] : kEmptyMap) { sum += i; }
