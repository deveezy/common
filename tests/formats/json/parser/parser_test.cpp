...
TEST(JsonStringParser, Int64) {
  ...
  EXPECT_THROW_TEXT((fjp::ParseToType<int, fjp::IntParser>("3.01")), fjp::ParseError,
...
TEST(JsonStringParser, Double) {
  ...
  EXPECT_THROW_TEXT((fjp::ParseToType<double, fjp::DoubleParser>("123.456a")), fjp::ParseError,
      "Parse error at pos 7, path '': The document root must not "
      "be followed by other values.");
  EXPECT_THROW_TEXT((fjp::ParseToType<double, fjp::DoubleParser>("[]")), fjp::ParseError,
...
TEST(JsonStringParser, Int64Overflow) {
  ...
  EXPECT_THROW_TEXT((fjp::ParseToType<int64_t, fjp::Int64Parser>(input)), fjp::ParseError,
...
TEST(JsonStringParser, EmptyObjectKey) {
  ...
  fjp::ParserState state;
  state.PushParser(obj_parser);
  EXPECT_THROW_TEXT(state.ProcessInput(input), fjp::ParseError,
...
protected:
  ...
  void EndObject() override {
    if (!has_field_) { throw fjp::InternalParseError("Missing required field 'field'"); }
...
TEST(JsonStringParser, IntObjectNoField) {
  ...
  EXPECT_THROW_TEXT((fjp::ParseToType<IntObject, IntObjectParser>(input)), fjp::ParseError,
...
TYPED_TEST(JsonStringParserMap, Invalid) {
  ...
  EXPECT_THROW_TEXT(state.ProcessInput(R"({{"key": 1}})"), fjp::ParseError,
...
std::string GenerateNestedJson(std::size_t depth) {
  ...
  for (std::size_t i = 0; i < depth; ++i) { result.append("{\"o\":"); }
  result.append("{}");
  for (std::size_t i = 0; i < depth; ++i) {
    result.push_back('}');
    if (i + 2 == depth) { result.append(",\"nonsense\""); }
  }
