...
#include "../common/member_modify_test.hpp"

namespace {

const auto kDoc = formats::json::MakeObject("key1", 1, "key2", "val", "key3", formats::json::MakeObject("sub", -1),
...
struct MemberModify<formats::json::ValueBuilder> : public ::testing::Test {
  ...
  static_assert(
      std::is_assignable_v<decltype(*std::declval<formats::json::ValueBuilder>().begin()), formats::json::ValueBuilder>,
