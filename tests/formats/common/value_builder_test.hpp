...
TYPED_TEST_P(InstantiationDeathTest, NanInf) {
  ...
  if constexpr (!std::is_same<formats::bson::ValueBuilder, ValueBuilder>::value) {
...
TYPED_TEST_P(CommonValueBuilderTests, Resize) {
  ...
  for (std::size_t i = 0; i < new_size; ++i) { builder[i] = i; }
  const auto value = builder.ExtractValue();
  for (std::size_t i = 0; i < new_size; ++i) { EXPECT_EQ(i, value[i].template As<std::size_t>()); }
}
...
TYPED_TEST_P(CommonValueBuilderTests, Chars) {
  ...
  value_chars[0] = 'X';
  ASSERT_EQ("Some string in std::string_view", builder.ExtractValue().template As<std::string>());
}
...
REGISTER_TYPED_TEST_SUITE_P(
    CommonValueBuilderTests, StringStrongTypedef, Resize, StringView, Chars, Nullptr, ImplicitType);
