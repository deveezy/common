...
TYPED_TEST_P(Conversion, Bool) {
  ...
  bool ethalon = false;
  for (const auto &elem : {value["a"][0], value["a"][1], value["et"], value["ef"]}) {
    ethalon = !ethalon;
...
TYPED_TEST_P(Conversion, Double) {
  ...
  for (const auto &[elem, ethalon] : {
    ...
    if (ethalon > -1.0) {
      EXPECT_EQ(static_cast<size_t>(ethalon), elem.template ConvertTo<size_t>());
    } else {
      EXPECT_ANY_THROW(elem.template ConvertTo<size_t>()) << "ethalon=" << ethalon;
    }
    EXPECT_DOUBLE_EQ(ethalon, elem.template ConvertTo<double>());
...
TYPED_TEST_P(Conversion, Utf8) {
  ...
  for (const auto &[elem, ethalon] : {
    ...
    EXPECT_THROW(elem.template ConvertTo<int32_t>(), Exception) << "ethalon=" << ethalon;
    // NOLINTNEXTLINE(misc-throw-by-value-catch-by-reference)
    EXPECT_THROW(elem.template ConvertTo<int64_t>(), Exception) << "ethalon=" << ethalon;
    // NOLINTNEXTLINE(misc-throw-by-value-catch-by-reference)
    EXPECT_THROW(elem.template ConvertTo<size_t>(), Exception) << "ethalon=" << ethalon;
    // NOLINTNEXTLINE(misc-throw-by-value-catch-by-reference)
    EXPECT_THROW(elem.template ConvertTo<double>(), Exception) << "ethalon=" << ethalon;
    EXPECT_EQ(ethalon, elem.template ConvertTo<std::string>()) << "ethalon=" << ethalon;
  }
}
...
TYPED_TEST_P(Conversion, Containers) {
  ...
  // possible false positive because of conditional in catch?
  // NOLINTNEXTLINE(misc-throw-by-value-catch-by-reference)
  EXPECT_THROW((value["a"].template ConvertTo<std::unordered_map<std::string, int>>()), Exception);
  // NOLINTNEXTLINE(misc-throw-by-value-catch-by-reference)
  EXPECT_THROW(value["d"].template ConvertTo<std::vector<int>>(), Exception);

  auto arr = value["a"].template ConvertTo<std::vector<int>>();
  for (int i = 0; i < static_cast<int>(arr.size()); ++i) { EXPECT_EQ(i, arr[i]) << "mismatch at position " << i; }

  auto umap = value["d"].template ConvertTo<std::unordered_map<std::string, int>>();
  EXPECT_EQ(1, umap["one"]);
  EXPECT_EQ(2, umap["two"]);

  EXPECT_TRUE((value["n"].template ConvertTo<std::unordered_map<std::string, int>>().empty()));
...
TYPED_TEST_P(Conversion, ContainersSerialize) {
  ...
  auto test_umap = inval["d"].template ConvertTo<std::unordered_map<std::string, int>>();
  ...
  for (size_t i = 0; i < ref_vector.size(); ++i) { EXPECT_EQ(ref_vector[i], inval["a"][i].template As<int>()); }
}
...
inline ParsableOnly Parse(const Value &value, formats::parse::To<ParsableOnly>) {
...
REGISTER_TYPED_TEST_SUITE_P(Conversion,
    ...
    Missing, Null, Bool, Double, Int32, Int64, Utf8, Containers, ContainersSerialize, ParseFallback);
