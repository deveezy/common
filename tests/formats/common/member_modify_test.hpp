...
TYPED_TEST_P(MemberModify, BuildNewValueEveryTime) {
  EXPECT_FALSE(this->GetBuiltValue().DebugIsReferencingSameMemory(this->GetBuiltValue()));
}
...
TYPED_TEST_P(MemberModify, ArrayResize) {
  ...
  for (size_t i = 0; i < 4; ++i) { EXPECT_EQ(this->GetBuiltValue()["key4"][i].template As<int>(), i + 1); }
...
TYPED_TEST_P(MemberModify, ArrayPushBack) {
  ...
  const auto size = 5;
  for (auto i = 0; i < size; ++i) { this->builder_.PushBack(i); }
  EXPECT_EQ(this->GetBuiltValue().IsEmpty(), !size);
  EXPECT_EQ(this->GetBuiltValue().GetSize(), size);

  for (auto i = 0; i < size; ++i) { EXPECT_EQ(this->GetBuiltValue()[i].template As<int>(), i); }
}
...
TYPED_TEST_P(MemberModify, ObjectIteratorModify) {
...

  {
    ...
    for (const auto &v : values) { sum -= v.template As<int>(); }
    EXPECT_EQ(sum, 0);
  }
}
...
TYPED_TEST_P(MemberModify, ArrayIteratorRead) {
  ...
  for (auto i = 0; i < size; ++i) { this->builder_[i] = i; }

  auto it = this->GetBuiltValue().begin();
  for (auto i = 0; i < size; ++i, ++it) { EXPECT_EQ(it->template As<int>(), i) << "Failed at index " << i; }
...
TYPED_TEST_P(MemberModify, ArrayIteratorModify) {
  ...
  const size_t offset = 3;
  {
    ...
    for (auto i = 0; i < size; ++i) { this->builder_[i] = 0; }

    auto it = this->builder_.begin();
    for (auto i = offset; it != this->builder_.end(); ++it, ++i) { *it = i; }
  }

  {
    auto it = this->GetBuiltValue().begin();
    for (auto i = 0; i < size; ++i, ++it) { EXPECT_EQ(it->template As<int>(), i + offset) << "Failed at index " << i; }
  }
  EXPECT_EQ(this->builder_.IsEmpty(), !size);
  EXPECT_EQ(this->builder_.GetSize(), size);
}
...
TYPED_TEST_P(MemberModify, TypeCheckMinMax) {
  ...
  EXPECT_EQ(v["uint8_t"].template As<uint64_t>(), std::numeric_limits<uint8_t>::max());
  EXPECT_EQ(v["int16_t"].template As<int>(), std::numeric_limits<int16_t>::min());
  EXPECT_EQ(v["uint16_t"].template As<uint64_t>(), std::numeric_limits<uint16_t>::max());
  EXPECT_EQ(v["int32_t"].template As<int>(), std::numeric_limits<int32_t>::min());
  EXPECT_EQ(v["uint32_t"].template As<uint64_t>(), std::numeric_limits<uint32_t>::max());
  EXPECT_EQ(v["int64_t"].template As<int64_t>(), std::numeric_limits<int64_t>::min());
  EXPECT_EQ(v["uint64_t"].template As<uint64_t>(), std::numeric_limits<uint64_t>::max());
  EXPECT_EQ(v["size_t"].template As<size_t>(), std::numeric_limits<size_t>::max());
  EXPECT_EQ(v["long"].template As<int64_t>(), std::numeric_limits<long>::min());
  EXPECT_EQ(v["long long"].template As<int64_t>(), std::numeric_limits<long long>::min());
...
TYPED_TEST_P(MemberModify, IsCheckObjectValidFunction) {
  ...
  }
  {
    ...
    // possible false positive because of conditional in catch?
    // NOLINTNEXTLINE(misc-throw-by-value-catch-by-reference)
    EXPECT_THROW(builder.ExtractValue()["a"].CheckObject(), TypeMismatchException);
  }
  {
...
REGISTER_TYPED_TEST_SUITE_P(MemberModify,
    ...
    PushBackFromExisting, PushBackFromBefore, PushBackFromIterator, CopyExistingMember,
    CopyExistingMemberByIt, /* ModifyRemovedNode, */
    ContainerTypeChangeToScalar, ContainerTypeChangeArrToObj, ContainerTypeChangeObjToArr,

    PushBackWrongTypeThrows, ExtractFromSubBuilderThrows, ObjectIteratorModify, MemberEmpty, MemberCount,

    NonArrayThrowIsEmpty, NonArrayThrowGetSize, ArrayIteratorRead, ArrayIteratorModify, CreateSpecificType,
    IteratorOutlivesRoot,

    SubdocOutlivesRoot, MoveValueBuilder, CheckSubobjectChange, TypeCheckMinMax, CannotBuildFromMissing,
