...
TYPED_TEST_P(Parsing, ContainersCtr) {
  ...
  EXPECT_TRUE((value.template As<std::unordered_map<std::string, int>>().empty()));
}
...
TYPED_TEST_P(Parsing, AsDefaulted) {
  ...
  EXPECT_EQ(json["missing"].template As<std::vector<int>>({}), std::vector<int> {});
}
...
REGISTER_TYPED_TEST_SUITE_P(Parsing,
