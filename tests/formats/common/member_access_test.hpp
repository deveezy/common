...
TYPED_TEST_P(MemberAccess, Algorithms) {
  auto it = std::find_if(this->doc_.begin(), this->doc_.end(),
      [](const auto &v) { return v.IsString() && v.template As<std::string>() == "val"; });
  ...
  ++it_new;
  EXPECT_NE(it_new, it);
  if (this->doc_.end() != it_new) { EXPECT_NE(it_new.GetName(), it.GetName()); }

  EXPECT_EQ(it->template As<std::string>(), "val");

  ++it;
  EXPECT_EQ(it_new, it);
  if (this->doc_.end() != it_new) { EXPECT_EQ(it_new.GetName(), it.GetName()); }
}
...
REGISTER_TYPED_TEST_SUITE_P(MemberAccess,

    ChildBySquareBrackets, ChildBySquareBracketsTwice, ChildBySquareBracketsMissing, ChildBySquareBracketsMissingTwice,
    ChildBySquareBracketsArray, ChildBySquareBracketsBounds,

    Items, IterateMemberNames, IterateAndCheckValues, IterateMembersAndCheckKey4, IterateMembersAndCheckKey4Index,
    IterateMembersAndCheckKey4IndexPostincrement, Algorithms,

    CheckPrimitiveTypes, CheckPrimitiveTypeExceptions, MemberPaths, MemberPathsByIterator, MemberEmpty, MemberCount,
    HasMember, CopyMoveSubobject, IteratorOnNull,

    IteratorOnMissingThrows, CloneValues, CreateEmptyAndAccess, Subfield, ValueAssignment, ConstFunctionsOnMissing,
    AsWithDefault, RootAndPathOfCloned);
