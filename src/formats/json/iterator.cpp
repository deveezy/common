...
const formats::json::Value &GetValue(const formats::json::Value &container) { return container; }

formats::json::Value &GetValue(formats::json::Value &container) { return container; }

const formats::json::Value &GetValue(const impl::MutableValueWrapper &container) { return *container; }
...
template <typename Traits, IteratorDirection Direction>
Iterator<Traits, Direction> &Iterator<Traits, Direction>::operator=(Iterator<Traits, Direction> &&other) noexcept {
  container_ = std::move(other.container_);
  type_      = other.type_;
  pos_       = other.pos_;
  current_.reset();
...
Iterator<Traits, Direction> Iterator<Traits, Direction>::operator++(int) {
  ...
  return Iterator<Traits, Direction>(ContainerType {container_}, type_, prev_pos);
}
...
template <typename Traits, IteratorDirection Direction>
typename Iterator<Traits, Direction>::pointer Iterator<Traits, Direction>::operator->() const {
  return &**this;
}

template <typename Traits, IteratorDirection Direction>
bool Iterator<Traits, Direction>::operator==(const Iterator<Traits, Direction> &other) const {
...
size_t Iterator<Traits, Direction>::GetIndex() const {
  if (type_ == impl::Type::arrayValue) { return pos_; }
...
void Iterator<Value::IterTraits, IteratorDirection::kForward>::UpdateValue() const {
  ...
  if (type_ == impl::Type::arrayValue) {
    current_.emplace(Value::EmplaceEnabler {}, container_.holder_, container_.root_ptr_for_path_,
        &container_.value_ptr_->Begin()[pos_], container_.depth_ + 1);
  } else {
    current_.emplace(Value::EmplaceEnabler {}, container_.holder_, container_.root_ptr_for_path_,
        &container_.value_ptr_->MemberBegin()[pos_].value, container_.depth_ + 1);
  }
}
...
void Iterator<ValueBuilder::IterTraits, IteratorDirection::kForward>::UpdateValue() const {
  ...
  if (type_ == impl::Type::arrayValue) {
    ...
    current_.emplace(ValueBuilder::EmplaceEnabler {},
        container_.WrapMember(GetName(), container_->value_ptr_->MemberBegin()[pos_].value));
  }
}
