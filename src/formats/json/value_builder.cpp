...
rapidjson::Type ToNativeType(Type type) {
  switch (type) {
    case Type::kNull: return rapidjson::kNullType;
    case Type::kArray: return rapidjson::kArrayType;
    case Type::kObject: return rapidjson::kObjectType;
    default:
      ASSERT_MSG(false, "No mapping from formats::json::Type to rapidjson::Type found");
...
ValueBuilder &ValueBuilder::operator=(const ValueBuilder &other) {
  ...
  if ((value_->IsArray() || value_->IsObject()) && value_->GetSize() != 0) { value_.OnMembersChange(); }
...
ValueBuilder::ValueBuilder(formats::json::Value &&other) {
  ...
  if (other.IsUniqueReference()) value_->GetNative() = std::move(other.GetNative());
  else
...
ValueBuilder::ValueBuilder(impl::MutableValueWrapper value) noexcept : value_(std::move(value)) {}
...
void ValueBuilder::Remove(std::string_view key) {
  value_->CheckObject();
  if (value_->GetNative().RemoveMember(impl::MakeJsonStringViewValue(key))) { value_.OnMembersChange(); }
}
...
void ValueBuilder::Resize(std::size_t size) {
  ...
  if (size > old_capacity) {
    ...
    if (old_capacity) { value_.OnMembersChange(); }
  }
  for (size_t curr_size = native.Size(); curr_size > size; --curr_size) { native.PopBack(); }
...
void ValueBuilder::PushBack(ValueBuilder &&bld) {
  ...
  const auto checked_push_back = [this, &native](auto &&value) {
    ...
    if (old_capacity && old_capacity != native.Capacity()) { value_.OnMembersChange(); }
  };
...
impl::Value &ValueBuilder::AddMember(std::string_view key, CheckMemberExists check_exists) {
  ...
  if (native.IsNull()) {
    ...
    if (it != native.MemberEnd()) { return it->value; }
  }

  // notify wrapper when members capacity (and thus location) changes
  const auto old_capacity = native.MemberCapacity();
  native.AddMember(impl::Value(key.data(), key.size(), g_allocator), impl::Value {}, g_allocator);
  if (old_capacity && old_capacity != native.MemberCapacity()) { value_.OnMembersChange(); }
