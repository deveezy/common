...
std::string ToLimited(std::string_view sw) {
  if (sw.size() > 128) return std::string(sw.substr(0, 128)) + "... (truncated)";
  else
...
void ParserState::ProcessInput(std::string_view sw) {
  ...
  try {
    while (!reader.IterativeParseComplete()) {
      if (stack.empty()) { throw InternalParseError("Symbols after end of document"); }
  ...
  } catch (const ParseError &) { throw; } catch (const std::exception &e) {
  ...
  if (!stack.empty()) { throw ParseError(is.Tell(), "", "data is expected after the end of file"); }
}

BaseParser &ParserState::GetTopParser() const {
  ASSERT(!impl_->stack.empty());
  return *impl_->stack.back().parser;
}

void ParserState::PopMe([[maybe_unused]] BaseParser &parser) {
  ASSERT(!impl_->stack.empty());
  ASSERT(&parser == impl_->stack.back().parser);
