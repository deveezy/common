...
bool WriteEnd(Handler &handler, const Cell &cell) {
  ...
  return cell.Visit([&handler, size = cell.GetSize()](
                        const SubRange<MemberIterator> &) mutable -> bool { return handler.EndObject(size); },
      [&handler, size = cell.GetSize()](
          const SubRange<ArrayIterator> &) mutable -> bool { return handler.EndArray(size); });
}
...
void EmplaceObject(Stack<kProcessing> &stack, typename ValueTypes<kProcessing>::Value &value) {
  if constexpr (kProcessing == ObjectProcessing::kInplaceSorting) { InplaceSortObjectChildren(value); }
...
bool WriteStartAndEnterValue(Stack<kProcessing> &stack, Handler &handler) {
  ...
  Value &value     = stack.back().Visit(
      [&was_written, &handler](const SubRange<MemberIterator> &member_sub_range) mutable -> Value     &{
        was_written =
            handler.Key(member_sub_range.begin->name.GetString(), member_sub_range.begin->name.GetStringLength(),
                    /*copy=*/true);
...
bool AcceptNoRecursion(typename ValueTypes<kProcessing>::Value &process_value, Handler &handler) {
  ...
  while (true) {
    if (stack.back().IsEnd()) {
      ...
      if (!impl::WriteEnd(handler, stack.back())) { return false; }
      stack.pop_back();

      stack.back().Next();
    } else {
      if (!impl::WriteStartAndEnterValue(stack, handler)) { return false; }
    }
  }
}
