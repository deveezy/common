...
public:
  ...
  TreeIterFrame(const Value *container, std::size_t index) : container_ {container}, current_index_ {index} {
    if (container->IsArray()) end_ = container->Size();
    else if (container->IsObject())
      end_ = container->MemberCount();
    else {
      ASSERT_MSG(false, "TreeIterFrame called on non-container element");
...
using TreeStack = boost::container::small_vector<TreeIterFrame, kInitialStackDepth>;
