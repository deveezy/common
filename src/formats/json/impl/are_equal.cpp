...
std::string_view AsStringView(const Value &value) { return {value.GetString(), value.GetStringLength()}; }
...
public:
  ...
  std::pair<ValuePtr, ValuePtr> Pop() noexcept {
    ASSERT(start_index_ < buffer_.size());
...
SurfaceCheckResult SurfaceCheck(const Value *lhs, const Value *rhs) {
  if (lhs == rhs) { return SurfaceCheckResult::kPass; }

  if (lhs == nullptr || rhs == nullptr || lhs->GetType() != rhs->GetType()) { return SurfaceCheckResult::kDeny; }

  if (lhs->IsObject()) {
    if (lhs->MemberCount() != rhs->MemberCount()) { return SurfaceCheckResult::kDeny; }
    return SurfaceCheckResult::kNeedsDeeperCheck;
  }

  if (lhs->IsArray()) {
    if (lhs->Size() != rhs->Size()) { return SurfaceCheckResult::kDeny; }
    return SurfaceCheckResult::kNeedsDeeperCheck;
  }
...
bool AreNotEqualObjectsThin(const Value *lhs, const Value *rhs, ParallelTraverseQueue &traverse) {
  ...
  for (std::size_t i = 0; i < size; ++i) {
    ...
    for (; j < size; ++j) {
      ...
      if (l_name != AsStringView(r_member.name)) { continue; }

      auto surf = SurfaceCheck(&l_member.value, &r_member.value);
      if (surf == SurfaceCheckResult::kPass) { break; }
      if (surf == SurfaceCheckResult::kDeny) { return true; }
...
bool AreNotEqualObjects(const Value *lhs, const Value *rhs, ParallelTraverseQueue &traverse) {
  // both lhs and rhs are expected to be objects with the same size

  const auto size = lhs->MemberCount();
  if (size <= 32) { return AreNotEqualObjectsThin(lhs, rhs, traverse); }

  boost::container::small_vector<std::pair<std::string_view, const Value *>, 64> lhs_object_kvs;
  ...
  std::sort(lhs_object_kvs.begin(), lhs_object_kvs.end(),
      [](const auto &l, const auto &r) { return PartiallyOrderedStringCmp(l.first, r.first); });

  const auto r_members = rhs->MemberBegin();
  for (std::size_t i = 0; i < size; ++i) {
    const auto r_name          = AsStringView(r_members[i].name);
    const auto matching_lhs_it = std::lower_bound(lhs_object_kvs.begin(), lhs_object_kvs.end(), r_name,
        [](const auto &check, const auto &key) { return PartiallyOrderedStringCmp(check.first, key); });
    if (matching_lhs_it == lhs_object_kvs.end() || matching_lhs_it->first != r_name) { return true; }

    auto surf = SurfaceCheck(matching_lhs_it->second, &r_members[i].value);
    if (surf == SurfaceCheckResult::kPass) { continue; }
    if (surf == SurfaceCheckResult::kDeny) { return true; }
...
bool AreEqual(const Value *lhs_root, const Value *rhs_root) {
  ...
  if (root_surf == SurfaceCheckResult::kPass) { return true; }
  if (root_surf == SurfaceCheckResult::kDeny) { return false; }

  ParallelTraverseQueue traverse;
  traverse.Push(lhs_root, rhs_root);

  while (!traverse.Empty()) {
    const auto [lhs, rhs] = traverse.Pop();  // Guaranteed to have the same object or array type and
                                             // the same size
    if (lhs->IsArray()) {
      ...
      for (std::size_t i = 0; i < lhs->Size(); ++i) {
        ...
        if (surf == SurfaceCheckResult::kPass) { continue; }
        if (surf == SurfaceCheckResult::kDeny) { return false; }
