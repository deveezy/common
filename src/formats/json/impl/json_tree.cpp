...
const Member *MemberFromValue(const Value *val) {
  return reinterpret_cast<const Member *>(reinterpret_cast<const char *>(val) - offsetof(Member, value));
}
...
std::size_t FastLookup(const TValue *member, const TValue *first, std::size_t size) {
  return member >= first && member < first + size ? member - first : kLookupFailed;
}
...
bool ProcessContainer(formats::json::impl::TreeStack &stack, const Value *container, const TValue *member,
    int node_depth, const TValue *first, std::size_t size, const TValidateAddress &extra_check) {
...
std::string MakePath(const Value *root, const Value *node, int node_depth) {
  ...
  for (;;) {
    ...
    if (value->IsObject() && value->MemberCount() > 0) {
      if (ProcessContainer(stack, value, MemberFromValue(node), node_depth, &*value->MemberBegin(),
              value->MemberCount(), [node](const Member *m) { return &m->value == node; }))
        break;
    } else if (value->IsArray() && value->Size() > 0) {
      if (ProcessContainer(stack, value, node, node_depth, &*value->Begin(), value->Size(),
...
std::string ExtractPath(const TreeStack &stack) {
  ...
  for (size_t depth = 1; depth < stack.size(); depth++) {
    ...
    if (frame.container()->IsObject()) {
      ...
      common::AppendPath(path, std::string_view {name.GetString(), name.GetStringLength()});
