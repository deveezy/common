...
#include "impl/are_equal.hpp"
#include "impl/exttypes.hpp"
#include "impl/json_tree.hpp"
...
constexpr int64_t kMaxIntDouble {int64_t {1} << std::numeric_limits<double>::digits};
...
bool IsNonOverflowingIntegral(const double val) {
  if constexpr (sizeof(Int) >= sizeof(double)) {
    ...
    return val >= std::numeric_limits<Int>::min() && val <= std::numeric_limits<Int>::max() && IsIntegral(val);
  }
}
...
impl::Value MakeJsonStringViewValue(std::string_view view) { return impl::Value(view.data(), view.size()); }
...
Value::Value(EmplaceEnabler, const impl::VersionedValuePtr &root, const impl::Value *root_ptr_for_path,
...
Value::Value(EmplaceEnabler, const impl::VersionedValuePtr &root, impl::Value *root_ptr_for_path,
...
Value Value::operator[](std::string_view key) const {
  if (!IsMissing()) {
    CheckObjectOrNull();
    if (IsObject()) {
      auto it = GetNative().FindMember(impl::Value(::rapidjson::StringRef(key.data(), key.size())));
      if (it != GetNative().MemberEnd()) {
        return {EmplaceEnabler {}, holder_, root_ptr_for_path_, &it->value, depth_ + 1};
      }
    }

    return {EmplaceEnabler {}, holder_, root_ptr_for_path_, LazyDetachedPath {value_ptr_, depth_, key}};
  }

  return {EmplaceEnabler {}, holder_, root_ptr_for_path_, lazy_detached_path_.Chain(key)};
}

Value Value::operator[](std::size_t index) const {
  CheckInBounds(index);
  return {EmplaceEnabler {}, holder_, root_ptr_for_path_, &GetNative()[static_cast<int>(index)], depth_ + 1};
}
...
bool Value::IsInt() const noexcept {
  ...
  if (native.IsDouble()) { return IsNonOverflowingIntegral<int>(native.GetDouble()); }
  return false;
}
...
bool Value::IsInt64() const noexcept {
  ...
  if (native.IsDouble()) { return IsNonOverflowingIntegral<int64_t>(native.GetDouble()); }
  return false;
}
...
bool Value::IsUInt64() const noexcept {
  ...
  if (native.IsDouble()) { return IsNonOverflowingIntegral<uint64_t>(native.GetDouble()); }
  return false;
}
...
Value Value::Clone() const { return Value {impl::VersionedValuePtr::Create(GetNative(), g_allocator)}; }
...
void Value::CheckArrayOrNull() const {
  if (!IsNull() && !IsArray()) { throw TypeMismatchException(GetExtendedType(), impl::arrayValue, GetPath()); }
}

void Value::CheckObjectOrNull() const {
  if (!IsNull() && !IsObject()) { throw TypeMismatchException(GetExtendedType(), impl::objectValue, GetPath()); }
}

void Value::CheckObject() const {
  if (!IsObject()) { throw TypeMismatchException(GetExtendedType(), impl::objectValue, GetPath()); }
}
...
void Value::CheckInBounds(std::size_t index) const {
  CheckArrayOrNull();
  if (index >= GetSize()) { throw OutOfBoundsException(index, GetSize(), GetPath()); }
}
...
Value::LazyDetachedPath::LazyDetachedPath(LazyDetachedPath &&) noexcept = default;

Value::LazyDetachedPath &Value::LazyDetachedPath::operator=(const LazyDetachedPath &) = default;

Value::LazyDetachedPath &Value::LazyDetachedPath::operator=(LazyDetachedPath &&) noexcept = default;

std::string Value::LazyDetachedPath::Get(const impl::Value *root) const {
  if (parent_value_ptr_ == nullptr) { return formats::common::kPathRoot; }

  return formats::common::MakeChildPath(impl::MakePath(root, parent_value_ptr_, parent_depth_), virtual_path_);
}

Value::LazyDetachedPath Value::LazyDetachedPath::Chain(std::string_view key) const {
  LazyDetachedPath result {*this};
  result.virtual_path_ = formats::common::MakeChildPath(std::move(result.virtual_path_), key);
