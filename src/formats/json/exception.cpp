...
std::string MsgForType(TType actual, TType expected) {
  return fmt::format("Wrong type. Expected: {}, actual: {}", NameForType(expected), NameForType(actual));
}
...
std::string MsgForUnknownDiscriminator(std::string_view discriminator_value) {
  return fmt::format("Unknown discriminator field value '{}'", discriminator_value);
}
