...
using SchemaValidator = rapidjson::GenericSchemaValidator<impl::SchemaDocument,
    rapidjson::BaseReaderHandler<impl::UTF8, void>, rapidjson::CrtAllocator>;

}  // namespace impl

SchemaValidationException::SchemaValidationException(
    std::string_view msg, std::string_view path, std::string_view schema_path)
...
void Schema::ValidationResult::ThrowIfError() && {
  if (IsError()) { std::move(*this).GetError().Throw(); }
}
...
Schema::ValidationError Schema::ValidationResult::GetError() && {
  ...
  if (iter == error.MemberEnd()) { iter = error.MemberBegin(); }
