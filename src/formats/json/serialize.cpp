...
void CheckKeyUniqueness(const impl::Value *root) {
  ...
  for (;;) {
    ...
    if (value->IsObject()) {
      ...
      if (count > keys.size()) { keys.resize(count); }
      for (std::size_t i = 0; i < count; ++i) { keys[i] = AsStringView(begin[i].name); }
      ...
      if (cons_eq_element != keys.data() + count) {
        throw ParseException("Duplicate key: " + std::string(*cons_eq_element) + " at " + impl::ExtractPath(stack));
      }
    }
...
Value FromString(std::string_view doc) {
  if (doc.empty()) { throw ParseException("JSON document is empty"); }

  impl::Document json {&g_allocator};
  rapidjson::ParseResult ok = json.Parse < rapidjson::kParseDefaultFlags | rapidjson::kParseIterativeFlag |
  ...
  if (!ok) {
    ...
    throw ParseException(
        fmt::format("JSON parse error at line {} column {}: {}", line, column, rapidjson::GetParseError_En(ok.Code())));
  }
...
Value FromStream(std::istream &is) {
  if (!is) { throw BadStreamException(is); }

  rapidjson::IStreamWrapper in(is);
  impl::Document json {&g_allocator};
  rapidjson::ParseResult ok = json.ParseStream < rapidjson::kParseDefaultFlags | rapidjson::kParseIterativeFlag |
...
void Serialize(const Value &doc, std::ostream &os) {
  ...
  if (!os) { throw BadStreamException(os); }
}
...
Value FromFile(const std::string &path) {
  ...
  } catch (const std::exception &e) { throw ParseException(fmt::format("Parsing '{}' failed. {}", path, e.what())); }
}
