...
const std::array<std::string, kIndexCacheSize> kIndexCache = [] {
  ...
  for (size_t i = 0; i < kIndexCacheSize; i++) { arr[i] = GetIndexString(i); }
  return arr;
}();
...
std::string MakeChildPath(std::string &&parent, std::size_t index) {
  if (parent == kPathRoot) { parent.clear(); }

  AppendPath(parent, index);
  return std::move(parent);
}

std::string GetIndexString(size_t index) { return fmt::format(FMT_COMPILE("[{}]"), index); }
...
std::string_view Path::ToStringView() const { return IsRoot() ? kPathRoot : std::string_view {path_}; }

Path Path::MakeChildPath(std::string_view key) const { return Path(formats::common::MakeChildPath(path_, key)); }

Path Path::MakeChildPath(std::size_t index) const { return Path(formats::common::MakeChildPath(path_, index)); }
