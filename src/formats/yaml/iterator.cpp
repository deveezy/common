...
Iterator<iter_traits>::Iterator(
    const typename iter_traits::native_iter &iter, int index, const formats::yaml::Path &path)
    : iter_pimpl_(iter)
    , path_(path)
    , index_(index) {}
...
template <typename iter_traits>
typename Iterator<iter_traits>::reference Iterator<iter_traits>::operator*() const {
  UpdateValue();
  return *current_;
}

template <typename iter_traits>
typename Iterator<iter_traits>::pointer Iterator<iter_traits>::operator->() const {
  UpdateValue();
  return &**this;
}

template <typename iter_traits>
bool Iterator<iter_traits>::operator==(const Iterator<iter_traits> &other) const {
...
std::string Iterator<iter_traits>::GetName() const {
  if (index_ != -1) { throw TypeMismatchException(Type::kArray, Type::kObject, path_.ToStringView()); }
  return (*iter_pimpl_)->first.Scalar();
}

template <typename iter_traits>
uint32_t Iterator<iter_traits>::GetIndex() const {
  if (index_ == -1) { throw TypeMismatchException(Type::kObject, Type::kArray, path_.ToStringView()); }
  return index_;
}
