...
Value::Value(Value &&other, std::string path_prefix)
  ...
  if (!other.path_.IsRoot()) { throw PathPrefixException(other.path_.ToStringView(), path_.ToStringView()); }
}
...
void Value::CheckNotMissing() const {
  if (IsMissing()) { throw MemberMissingException(path_.ToStringView()); }
}

void Value::CheckArray() const {
  if (!IsArray()) { throw TypeMismatchException(GetExtendedType(), impl::arrayValue, path_.ToStringView()); }
}
...
void Value::CheckObject() const {
  if (!IsObject()) { throw TypeMismatchException(GetExtendedType(), impl::objectValue, path_.ToStringView()); }
}

void Value::CheckString() const {
  if (!IsString()) { throw TypeMismatchException(GetExtendedType(), impl::scalarValue, path_.ToStringView()); }
}
...
void Value::CheckInBounds(std::size_t index) const {
  CheckArrayOrNull();
  if (!(*value_pimpl_)[index]) { throw OutOfBoundsException(index, GetSize(), path_.ToStringView()); }
}
