...
#define RET_NAME(type) \
  if (Type::type == expected) { return #type; }
  RET_NAME(kNull)
  RET_NAME(kArray)
  RET_NAME(kObject)
  return "ERROR";
...
std::string MsgForType(std::string_view expected_type, const YAML::Node &value) {
  return fmt::format(
...
std::string MsgForIndex(size_t index, size_t size) {
  return fmt::format("Index {} of array of size {} is out of bounds", index, size);
}

std::string MsgForPathPrefix(std::string_view old_path, std::string_view prefix) {
  return fmt::format("Attempting to append path prefix '{}' to a non-empty path '{}'", prefix, old_path);
}
...
TypeMismatchException::TypeMismatchException(Type actual, Type expected, std::string_view path)
...
PathPrefixException::PathPrefixException(std::string_view old_path, std::string_view prefix)
