...
YAML::NodeType::value ToNative(Type t) {
  switch (t) {
    case Type::kArray: return YAML::NodeType::Sequence;
    case Type::kNull: return YAML::NodeType::Null;
    case Type::kObject: return YAML::NodeType::Map;
  }

  INVARIANT(false, "Unexpected YAML type");
}
...
ValueBuilder::ValueBuilder(const formats::yaml::Value &other) { NodeDataAssign(other); }

ValueBuilder::ValueBuilder(formats::yaml::Value &&other) { NodeDataAssign(other); }

ValueBuilder::ValueBuilder(
    EmplaceEnabler, const YAML::Node &value, const formats::yaml::Path &path, const std::string &key)
...
ValueBuilder ValueBuilder::MakeNonRoot(const YAML::Node &val, const formats::yaml::Path &path, const std::string &key) {
  return {EmplaceEnabler {}, val, path, key};
}

ValueBuilder ValueBuilder::MakeNonRoot(const YAML::Node &val, const formats::yaml::Path &path, size_t index) {
  return {EmplaceEnabler {}, val, path, index};
}

ValueBuilder ValueBuilder::operator[](const std::string &key) {
  if (value_.IsMissing()) { *value_.value_pimpl_ = YAML::Node(YAML::NodeType::value::Map); }
...
ValueBuilder::iterator ValueBuilder::begin() {
  ...
  return {value_.GetNative().begin(), value_.GetNative().IsSequence() ? 0 : -1, value_.path_};
}

ValueBuilder::iterator ValueBuilder::end() {
  value_.CheckObjectOrArrayOrNull();
  return {value_.GetNative().end(), value_.GetNative().IsSequence() ? static_cast<int>(value_.GetNative().size()) : -1,
      value_.path_};
}
...
bool ValueBuilder::HasMember(const char *key) const { return value_.HasMember(key); }

bool ValueBuilder::HasMember(const std::string &key) const { return value_.HasMember(key); }

void ValueBuilder::Resize(std::size_t size) {
  value_.CheckArrayOrNull();

  auto sequence_length = value_.GetNative().size();
  if (sequence_length == size) { return; }

  if (sequence_length < size) {
    for (; sequence_length != size; ++sequence_length) { value_.GetNative().push_back(YAML::Node {}); }
  } else {
    // YAML has problems with updating size after the shrink operation, so
    // instead of removing nodes, we copy them into a new one.
    YAML::Node new_node;
    for (std::size_t i = 0; i != size; ++i) { new_node[i] = (*value_.value_pimpl_)[i]; }
...
void ValueBuilder::PushBack(ValueBuilder &&bld) {
  value_.CheckArrayOrNull();
  value_.GetNative()[static_cast<int>(value_.GetSize())] = std::move(bld).value_.GetNative();
}

formats::yaml::Value ValueBuilder::ExtractValue() {
  if (!value_.IsRoot()) { throw Exception("Extract should be called only from the root builder"); }
...
void ValueBuilder::NodeDataAssign(const formats::yaml::Value &from) {
  if (from.IsMissing()) { throw MemberMissingException(from.GetPath()); }

  *value_.value_pimpl_ = YAML::Clone(*from.value_pimpl_);
}

void ValueBuilder::Copy(const ValueBuilder &from) { NodeDataAssign(from.value_); }
