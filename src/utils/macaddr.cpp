...
auto MacaddrOctetsFromString(const std::string &str) {
  ...
  const auto throw_exception = []() { throw std::invalid_argument("Error while converting MAC-address from string"); };

  const auto hex2_to_uchar = [&str, &octets, throw_exception](size_t index, size_t octets_counter) {
    std::string out;
    auto result = utils::encoding::FromHex(str.substr(index, 2), out);
    if (result < 2) { throw_exception(); }
...
Macaddr MacaddrFromString(const std::string &str) {
  ...
  if (octets_counter != octets.size()) { throw std::invalid_argument(std::to_string(octets_counter)); }
...
std::string Macaddr8ToString(Macaddr8 macaddr) {
  const auto octets = macaddr.GetOctets();
  return fmt::sprintf("%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x", octets[0], octets[1], octets[2], octets[3], octets[4],
