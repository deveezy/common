...
#endif
  cctz::time_zone tz;
  if (!load_time_zone(tzname, &tz)) { return std::nullopt; }
  return tz;
}

cctz::time_zone GetTimezone(const std::string &tzname) {
  auto tz = GetOptionalTimezone(tzname);
  if (!tz.has_value()) { throw TimezoneLookupError(tzname); }
  return *tz;
}
...
std::optional<std::chrono::system_clock::time_point> OptionalStringtime(
    const std::string &timestring, const cctz::time_zone &timezone, const std::string &format) {
  std::chrono::system_clock::time_point tp;
  if (cctz::parse(format, timestring, timezone, &tp)) { return tp; }
  return {};
}
...
std::chrono::system_clock::time_point DoGuessStringtime(
  ...
  for (const auto &format : formats) {
    ...
    if (optional_tp) { return *optional_tp; }
  }
...
bool IsTimeBetween(
    int hour, int min, int hour_from, int min_from, int hour_to, int min_to, bool include_time_to) noexcept {
  ...
  if (greater_that_time_from && lower_than_time_to) { return true; }
  ...
  if (hour == hour_from && min == min_from) { return true; }

  return false;
}

std::string Timestring(
    std::chrono::system_clock::time_point tp, const std::string &timezone, const std::string &format) {
  return cctz::format(format, tp, GetTimezone(timezone));
}

std::optional<std::chrono::system_clock::time_point> OptionalStringtime(
    const std::string &timestring, const std::string &timezone, const std::string &format) {
  auto tz = GetOptionalTimezone(timezone);
  if (!tz.has_value()) { return std::nullopt; }
...
std::chrono::system_clock::time_point Stringtime(
  ...
  if (!optional_tp) { throw DateParseError(timestring); }
  return *optional_tp;
}
...
std::uint32_t ParseDayTime(const std::string &str) {
  ...
  try {
    ...
    if (str.size() == 8) seconds = boost::numeric_cast<std::uint8_t>(boost::lexical_cast<int>(std::string(str, 6, 2)));

  } catch (const std::exception &ex) { throw std::invalid_argument(std::string("Failed to parse time from ") + str); }
