...
char GetNextChar(const char *&begin, const char *end) {
  if (begin == end) { throw std::runtime_error {"Invalid uuid string"}; }
  return *begin++;
}
...
unsigned char GetValue(char c) {
  ...
  if (pos == std::string_view::npos) { throw std::runtime_error {"Invalid uuid string"}; }
  return kValues[pos];
}
...
boost::uuids::uuid FromChars(const char *begin, const char *end) {
  ...
  if (has_open_brace) { c = GetNextChar(begin, end); }
  ...
  for (auto *byte_it = u.begin(); byte_it != u.end(); ++byte_it, ++byte_no) {
    if (byte_it != u.begin()) { c = GetNextChar(begin, end); }

    if (byte_no == 4) {
      has_dashes = IsDash(c);
      if (has_dashes) { c = GetNextChar(begin, end); }
...
boost::uuids::uuid BoostUuidFromString(std::string_view str) { return FromChars(str.data(), str.data() + str.size()); }
