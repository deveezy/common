...
int ToNative(OpenMode flags) {
  ...
  if ((flags & OpenFlag::kRead) && (flags & OpenFlag::kWrite)) {
    ...
    INVARIANT(false, "Specify at least one of kRead, kWrite in OpenFlags");
  }

  if (flags & OpenFlag::kCreateIfNotExists) {
    INVARIANT(flags & OpenFlag::kWrite, "Cannot use kCreateIfNotExists without kWrite in OpenFlags");
    result |= O_CREAT;
  }
  ...
  if (flags & OpenFlag::kTruncate) {
    INVARIANT(flags & OpenFlag::kWrite, "Cannot use kTruncate without kWrite in OpenFlags");
    INVARIANT(!(flags & OpenFlag::kExclusiveCreate), "Cannot use kTruncate with kExclusiveCreate in OpenFlags");
    result |= O_TRUNC;
  }

  if (flags & OpenFlag::kAppend) {
    INVARIANT(flags & OpenFlag::kWrite, "Cannot use kAppend without kWrite in OpenFlags");
    result |= O_APPEND;
  }
...
FileDescriptor::FileDescriptor(int fd) : fd_(fd) { ASSERT(fd != kNoFd); }

FileDescriptor FileDescriptor::Open(const std::string &path, OpenMode flags, std::filesystem::perms perms) {
  ASSERT(!path.empty());
...
FileDescriptor FileDescriptor::AdoptFd(int fd) noexcept {
  ASSERT_MSG(::fcntl(fd, F_GETFD) != -1, "This file descriptor is not valid");
  return FileDescriptor {fd};
}
...
void FileDescriptor::Seek(std::size_t offset_in_bytes) {
  while (true) {
    ...
    ASSERT(static_cast<std::size_t>(s) == offset_in_bytes);
    break;
  }
}

// NOLINTNEXTLINE(readability-make-member-function-const)
void FileDescriptor::FSync() { utils::CheckSyscall(::fsync(fd_), "calling ::fsync"); }

std::size_t FileDescriptor::GetSize() const { return GetFileStats(fd_).st_size; }
