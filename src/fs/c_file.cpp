...
struct FileDeleter {
  void operator()(std::FILE *file) noexcept {
    ...
    if (std::fclose(file) == -1) {
      ...
      std::cerr << "Error while closing a cache dump file: " << std::error_code(code, std::generic_category());
    }
  }
};
...
struct CFile::Impl {
  ...
  int GetFileDescriptor() const { return utils::CheckSyscall(::fileno(handle.get()), "calling ::fileno"); }
};
...
CFile::CFile(std::FILE *file) noexcept {
  ASSERT_MSG(file != nullptr, "This file is not valid");
  impl_->handle.reset(file);
}

CFile::CFile(const std::string &path, OpenMode flags, std::filesystem::perms perms) {
...
std::uint64_t CFile::GetPosition() const {
  ASSERT(IsOpen());
  const auto position = utils::CheckSyscall(::ftello(impl_->handle.get()), "calling ::ftello");
...
std::uint64_t CFile::GetSize() const {
  ...
  utils::CheckSyscall(::fstat(impl_->GetFileDescriptor(), &stats), "calling ::fstat");
